...
void ConstrainedDelaunay2D::createSuperTriangle_()
{
    ...
    activeTriangles_.emplace_back(superNodeIds_[0], superNodeIds_[1], superNodeIds_[2]);
}
...
void ConstrainedDelaunay2D::insertVertex_(size_t nodeId)
{
    ...
    std::vector<Triangle> newActiveTriangles;
                                           ...
                                           const std::vector<std::array<size_t, 2>>& boundary)
{
    ...
    {
        activeTriangles_.emplace_back(vertexNodeId, edge[0], edge[1]);
    }
}
...
void ConstrainedDelaunay2D::forceEdge_(size_t nodeId1, size_t nodeId2)
{
    ...
    {
        activeTriangles_.emplace_back(nodeId1, edge[0], edge[1]);
    }

    for (const auto& edge : side2Edges)
    {
        activeTriangles_.emplace_back(nodeId2, edge[0], edge[1]);
    }
}
...
std::vector<size_t> ConstrainedDelaunay2D::findIntersectingTriangles_(size_t nodeId1, size_t nodeId2) const
{
    ...
    for (size_t i = 0; i < activeTriangles_.size(); ++i)
    {
        ...
        if (tri.hasNode(nodeId1) && tri.hasNode(nodeId2))
        {
            continue;
        }
...
ConstrainedDelaunay2D::computeCircumcircle_(const Triangle& tri) const
{
    const Point2D& p0 = nodeCoords_.at(tri.nodes[0]);
    const Point2D& p1 = nodeCoords_.at(tri.nodes[1]);
    const Point2D& p2 = nodeCoords_.at(tri.nodes[2]);
