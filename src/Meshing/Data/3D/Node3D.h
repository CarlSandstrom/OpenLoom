 ...
 *
 * Nodes are either interior nodes (no geometry IDs) or boundary nodes
 * (with geometry IDs). Boundary nodes can be associated with multiple
 * edges/surfaces (e.g., corners where edges meet, or edges where surfaces
 * meet). The isBoundary() method returns true if the node has any geometry IDs.
 */
class Node3D
{
public:
    ...
    /// Returns true if this is a boundary node (has geometry IDs)
