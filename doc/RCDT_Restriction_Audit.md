...
Status: **step 1 (inventory) only.** Nothing here has been ablated yet. Every
"measured" cell cites an existing measurement from a commit message, a code
comment or a project memory — no new number in this document was produced by
this audit.
...
| 1.7 | **`isUniquePhaseBoundaryCandidate`** — **at most one** rival | Locality guard on 1.6 | `63e23b4`: barring all rivals measured zero improvement | **B, derived** — *not* fitted, contrary to the ticket's starting-point list | The "why one?" **is** answered, in `63e23b4`'s commit message and nowhere in the code: an ordinary interior edge's two genuine incident faces are on the *same* surface, so a face's own genuine partner is exactly one rival. 0 rivals would disqualify every legitimate interior face. **Action regardless of ablation: move that derivation into the code.** Retires when parity around the edge cycle replaces rival counting. |
...
| 1.11 | **`MAXIMUM_SAMPLES_PER_DIRECTION = 400`** | Caps tessellation memory/cost | none recorded | **C** | When this cap binds, the actual cell size **silently exceeds** the target and 1.9's guarantee is void with no diagnostic. Retires when either it is shown never to bind on any model in the golden set, or it logs when it does. **Cheapest probe in the whole audit.** |
...
| 1.14 | **`FaceRestriction::Unconfirmed`** | A third state distinguishing "no surface" from "no answer" | its own doc: "lets a classification failure be counted rather than silently becoming a missing face" | **C — dead as built** | **Verified statically this session: `Unconfirmed` is produced at four sites and consumed nowhere.** Every caller tests only `== Restricted`. The stated purpose is not realised: nothing counts it. Retirement condition: either wire it to a counter (it is the natural dependent variable step 2 asks for) or delete the state. Do not leave it as is. |
...
| 6.3 | **Size floor `length <= minimumEdgeLength_` → unrefinable** | Advertised as the stopping rule, "same reasoning as the other priorities" | **OPE-184 instrumented all seven refusal sites over the 65-defect repro: 107 refusals, size floor fired 0 times.** | **C — measured never to fire** | It is not the real cutoff. The real cutoff is `2 × minimumEdgeLength`, enforced invisibly by the proximity guard (6.5): 50 of 56 proximity refusals were blocked by the defect edge's *own* endpoint, at distance `length/2` by construction. Retirement: delete it, or keep it and delete the misleading comment — but only after 6.5 is understood, because removing it alone changes nothing. |
...
## Proposed step 2 — not started, for agreement

The ticket asks for a dependent variable that is a property of the algorithm,
not a count of symptoms. The proposal, in increasing cost:

1. **Static/zero-run**: nothing further — items 1–4 above are already settled
   by reading.
2. **One instrumented run per pinned model**, recording per classification
   query: which route answered (1.3 / 1.4 / 1.6 / 1.1 / `Unconfirmed`), and for
   the dual-edge route whether the dual edge was in a regime where the crossing
   test is meaningful. Plus: does 1.11's cap bind, and what cell size was
   actually achieved versus 1.9's target.
3. **Wire `LocalFeatureSize3D`** (OPE-181 stage 1, committed, tested, no
   consumer) as the denominator: sample spacing relative to local feature size
   at each query point. This is the quantity the ε-sample condition is stated
   in, and it is the half that makes the answer *principled* rather than merely
   *true of five analytic solids*.
4. **Only then** ablate the B and C rows, one per step, each with build →
   `ctest -LE slow` → `refactor-check.sh`, and `TIER=full` before finishing.

A golden that moves means the layer was not dead — report which model moved
and how, never re-bless.
