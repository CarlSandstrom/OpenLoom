...
|------|-------------|--------|
| 0.1 | Create `3D/General/`, `3D/Surface/`, `3D/Volume/` | **TODO** |
| 0.2 | Move general 3D infrastructure to `3D/General/`: `GeometryStructures3D`, `GeometryUtilities3D`, `DiscretizationResult3D`, `BoundaryDiscretizer3D`, `ConstraintRegistrar3D`, `ConstraintChecker3D`, `MeshingContext3D`, `MeshData3D` (if not already separate), `MeshOperations3D`, `MeshQueries3D`, `MeshVerifier3D`, `ElementGeometry3D`, `ElementQuality3D`, `MeshDebugUtils3D` | **TODO** |
| 0.3 | Decouple `BoundaryDiscretizer3D` from `MeshingContext3D` (currently depends on volume context; should work without tet data) | **TODO** |
...
| 0.5 | Move `FacetTriangulation`, `FacetTriangulationManager` to `3D/Surface/` | **TODO** |
| 0.6 | Move the four volume-specific files to `3D/Volume/`: `Delaunay3D`, `ConstrainedDelaunay3D`, `ShewchukRefiner3D`, `Shewchuk3DQualityController` | **TODO** |
| 0.7 | Update all `CMakeLists.txt` and `#include` paths; verify build passes | **TODO** |
...
### Phase 0: Folder restructuring
1. Create `3D/General/`, `3D/Surface/`, `3D/Volume/` and move files (see Phase 0 table above)
2. Extend `ElementGeometry3D` with triangle-in-3D operations
3. Decouple `BoundaryDiscretizer3D` from `MeshingContext3D`
4. Update `CMakeLists.txt` and `#include` paths; verify build passes
